...
struct ThreatHUD: View {
    ...
    private var consolidatedEvents: [SoundEvent] {
        ...
        // 1. THE VIP BOUNCER: Uses global lifespan AND the Pipeline's strict gate
        let revealedEvents = events.filter { event in
            ...
            // 🚨 TRUST THE PIPELINE: This strictly enforces leadInTime and confidence
            return event.isRevealed
        }
...
struct ThreatHUDItemInstance: View {
    ...
    var body: some View {
        ...
        let displayLabel = AppGlobals.localizeText(label: event.threatLabel)
