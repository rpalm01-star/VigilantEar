...
enum ThreatCategory: String, Sendable, CaseIterable {
...
struct SoundProfile {
    ...
    private static let emergencyAndSafety: [(keywords: [String], icon: String, color: Color, ceiling: Double, maxRange: Double, category: ThreatCategory, snaps: Bool, haptics: Int, cooldown: Double, minConf: Double, leadIn: Double, tail: Double)] = [
        ...
        (["siren", "ambulance_siren", "police_siren", "fire_engine_siren", "civil_defense_siren", "emergency_vehicle", "simulated_fire_truck"], "light.beacon.max.fill", .red, 0.50, 500, .emergency, true, 4, 0.10, 0.65, 0.00, 1.0),
        ...
        (["alarm", "smoke_detector", "alarm_clock", "glass_breaking"], "exclamationmark.triangle.fill", .red, 0.80, 150.0, .emergency, false, 3, 0.4, 0.9, 1.0, 2.0),
        (["person", "person_running", "person_walking"], "figure.walk.arrival", .red, 0.80, 150.0, .emergency, false, 2, 0.50, 0.60, 0.2, 2.0),
        (["knock", "door", "door_bell", "doorbell", "door_sliding"],
         // SF Symbol Name
         "hand.tap",
         // Color
         .cyan,
         ...
         0.25,
         // Outer max detection range (ft). Events further away than this are ignored.
         150.0,
         // Category
         .emergency,
         // 🚨 FIX 1: Turn off road snapping. This keeps the dot at the house.
         false,
         // Haptic pulse count
         2,
         // 🚨 FIX 2: Cooldown. Block echoes for 2 seconds so it doesn't chain-spawn.
         0.50,
         // Minimum confidence provided by Sound ML chip
         0.60,
         // 🚨 FIX 3: Lead time. Impulse sounds must reveal instantly (0.0).
         0.00,
         // 🚨 FIX 4: Tail Memory. Let the icon linger gracefully for 1.5s so the user can see it.
         1.0),
    ]
    ...
    private static let vehicles: [(keywords: [String], icon: String, color: Color, ceiling: Double, maxRange: Double, category: ThreatCategory, snaps: Bool, haptics: Int, cooldown: Double, minConf: Double, leadIn: Double, tail: Double)] = [
        // Keywords
        (["car", "car_horn", "car_passing_by", "race_car", "truck", "bus", "motorcycle", "traffic_noise", "engine", "engine_accelerating_revving", "engine_starting", "engine_idling", "engine_knocking", "vehicle_skidding", "lawn_mower"],
         // SF Symbol Name
         "car.fill",
         // Color
         .blue,
         ...
         2.0),
        (["train", "rail_transport", "door_slam", "air_horn", "train_whistle", "foghorn", "train_horn", "railroad_car", "train_wheels_squealing", "subway_metro", "aircraft", "helicopter", "airplane", "boat_water_vehicle", "sailing", "rowboat_canoe_kayak", "motorboat_speedboat"], "tram.fill.tunnel", .blue, 0.15, 600.0, .ignored, false, 0, 0.8, 0.50, 1.0, 3.0),
        (["bicycle","bicycle_bell"], "bicycle", .blue, 0.65, 250.0, .medium, false, 0, 1.5, 0.40, 0.5, 0.5),
    ]
    ...
    private static let humanSounds: [(keywords: [String], icon: String, color: Color, ceiling: Double, maxRange: Double, category: ThreatCategory, snaps: Bool, haptics: Int, cooldown: Double, minConf: Double, leadIn: Double, tail: Double)] = [
        ...
        (["telephone", "telephone_bell_ringing"],
         // SF Symbol Name
         "phone.fill",
         // Color
         .pink,
         ...
         // 🚨 FIX 3: Lead time. Impulse sounds must reveal instantly (0.0).
         0.00,
         // 🚨 FIX 4: Tail Memory. Let the icon linger gracefully for x seconds so the user can see it.
         0.25)
    ]
    ...
    private static let household: [(keywords: [String], icon: String, color: Color, ceiling: Double, maxRange: Double, category: ThreatCategory, snaps: Bool, haptics: Int, cooldown: Double, minConf: Double, leadIn: Double, tail: Double)] = [
        ...
        (["household", "cutlery_silverware", "dishes_pots_pans", "chopping_food", "frying_food", "blender", "water_tap_faucet", "bathtub_filling_washing", "hair_dryer", "drawer_open_close", "toothbrush", "vacuum_cleaner", "electric_shaver", "zipper"], "house.fill", .mint, 0.40, 40.0, .ignored, false, 0, 1.6, 0.65, 0.8, 1.5),
        ...
        (["liquid_splashing", "sink_filling_washing", "liquid_sloshing", "liquid_squishing", "liquid_dripping", "liquid_trickle_dribble", "liquid_filling_container", "liquid_spraying", "water_pump", "underwater_bubbling", "whoosh_swoosh_swish", "thump_thud", "crushing", "crumpling_crinkling", "tearing", "click"], "drop.fill", .mint, 0.45, 50.0, .ignored, false, 0, 2.2, 0.65, 0.5, 1.5),
    ...
    public static let rawRegistry: [(keywords: [String], icon: String, color: Color, ceiling: Double, maxRange: Double, category: ThreatCategory, snaps: Bool, haptics: Int, cooldown: Double, minConf: Double, leadIn: Double, tail: Double)] = {
...
// MARK: - SoundML Integration
extension SoundProfile {
    
    static var soundMLGroups: [SoundGroup] {
        rawRegistry.flatMap { entry in
            entry.keywords.compactMap { keyword in
                guard let soundType = SoundType(rawValue: keyword) else { return nil }
                return SoundGroup(soundType, threshold: entry.minConf)
            }
        }
    }
    
    // MARK: - Runtime Editable Profiles (for the TNG panel)
    static var allEditableProfiles: [EditableSoundProfile] {
        rawRegistry.map { entry in
            EditableSoundProfile(
                displayName: entry.keywords.first?.capitalized ?? "Unknown",
                originalKeywords: entry.keywords,
                category: entry.category,
                minimumConfidence: entry.minConf,
                leadInTime: entry.leadIn,
                tailMemory: entry.tail,
    ...
    // MARK: - Top-level Editable Profile (required for clean bindings)
