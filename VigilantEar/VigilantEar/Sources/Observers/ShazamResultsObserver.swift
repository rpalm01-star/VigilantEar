...
final class ShazamResultsObserver: NSObject, @unchecked Sendable, SHSessionDelegate {
    ...
    nonisolated func session(_ session: SHSession, didFind match: SHMatch) {
        ...
        AppGlobals.doLog(message: "♪ Shazam Match Found: \(title) by \(artist).", step: "SHAZAM")
        
        Task {
            await pipeline?.registerShazamResponse(title: title, artist: artist)
        }
    }
    ...
    nonisolated func session(_ session: SHSession, didNotFindMatchFor signature: SHSignature, error: Error?) {
        ...
        // 🚨 THE FIX: Call the dedicated failure handler
        Task {
            await pipeline?.handleShazamFailure()
        }
    }
}
