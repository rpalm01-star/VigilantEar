...
## 🌟 Key Features
 * **Real-Time Sound Classification:** Uses Apple's SoundAnalysis and Core ML to distinguish between motorcycles, sirens, and general city traffic, achieving hyper-efficient **0-1ms inference times** on the Neural Engine.
 * **Directional Tracking:** Guesses the heading of a moving sound source over a 5-second window using multi-microphone phase analysis and a highly calibrated **stereo microphone configuration**.
 ...
 * **Accessibility Alerts:** Provides haptic feedback and visual cues via a **real-time spatial Radar View interface** to alert users of approaching high-volume vehicles.
 * **Background Operation:** Runs as a background service with "Critical Alert" entitlements for safety-first notifications.
## 🛠️ Tech Stack (2026)
 * **Language:** Swift 6.0+ (featuring modern Swift 6 Concurrency and a Dependency Injection architecture)
 * **Frameworks:** SwiftUI, SoundAnalysis, Core ML
 * **Mapping:** Google Maps SDK for iOS
 * **Database:** SwiftData (for local event logging)
 * **Minimum Requirements:** iOS 18.0+, iPhone with 3-mic array configured for stereo capture (iPhone 13 or newer recommended)
...
### Installation
 ...
 3. Enable **"Audio, AirPlay, and Picture in Picture"** background mode + Critical Alerts entitlement.
